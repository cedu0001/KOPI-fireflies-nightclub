...
const Tables = ({ tables, reservations, selectedEvent, selectedTable, setSelectedTable }) => {
	const getTableStatus = (tableId) => {
		...
		const isReserved = reservations.some(
			(reservation) => reservation.eventId === Number(selectedEvent) && Number(reservation.table) === tableId,
		);
	...
	return (
		<article className="grid grid-cols-[repeat(5,minmax(75px,240px))] gap-y-(--space-m) justify-center mb-(--space-2xl) mt-(--space-l)">
			{tables.map((table) => {
				...
				return (
					...
					>
						...
						<p className="absolute inset-0 flex items-center justify-center font-bold">{table.tableNumber}</p>
					</div>
				);
			})}
		</article>
