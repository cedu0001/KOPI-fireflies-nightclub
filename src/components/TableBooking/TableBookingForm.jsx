...
const TableBookingForm = ({
	...
	const isReserved = (tableNumber) => {
		...
		return reservations.some(
			(reservation) =>
				Number(reservation.eventId) === Number(selectedEvent) && Number(reservation.table) === tableNumber,
		);
	};
	...
	return (
		<form className="mb-(--space-2xl)" action={formAction} key={state.message}>
			<h2>BOOK A TABLE</h2>
			{state?.message && (
				<div
				...
				>
					...
					<SelectContent>
						{Array.from({ length: 15 }).map((_, index) => {
							...
							return (
								<SelectItem key={tableNumber} value={String(tableNumber)} disabled={isReserved(tableNumber)}>
									Table {tableNumber}
									{isReserved(tableNumber) ? " (Reserved)" : ""}
								</SelectItem>
							);
						})}
					</SelectContent>
