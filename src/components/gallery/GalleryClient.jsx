				...
				<Dialog open={selectedIndex !== null} onOpenChange={() => setSelectedIndex(null)}>
					<DialogContent className="bg-primary h-fit w-fit !max-w-[95vw] border-none">
						<Button
							...
							onClick={() => setSelectedIndex((prev) => (prev === 0 ? galleryData.length - 1 : prev - 1))}
						>
						...
						<Button
							...
							onClick={() => setSelectedIndex((prev) => (prev === galleryData.length - 1 ? 0 : prev + 1))}
						>
						...
						<section className="flex flex-col">
							...
							<div>
								<h4 className="!py-0">{galleryData[selectedIndex].title}</h4>

								<p className="max-w-prose">{galleryData[selectedIndex].description}</p>
							</div>
						</section>
					</DialogContent>
				</Dialog>
